...
fprintf('ワーキングフォルダを選んでください: ');
WORKING_FOLDER = uigetdir('~','Select a working folder');
OUTPUT_FILE_NAME = datestr(now, 30);

NUMBER_OF_LEVELS = input('何段階で測定しますか？[32]: ');
if isempty(NUMBER_OF_LEVELS)
    NUMBER_OF_LEVELS=32;
end

SAMPLES_PER_LEVEL = input('各段階で何回ずつ繰り返し測光しますか？[3]: ');
if isempty(SAMPLES_PER_LEVEL)
    SAMPLES_PER_LEVEL=3;
end

MODE = input('ガンマキャリブレーション(g)？チェック(c)？[g]: ','s');
if isempty(MODE)
    MODE='g';
end

if MODE == 'c'
    INPUT_FILE_NAME = input('キャリブレーションデータのファイル名？', 's');
    GAMMA_CORRECTION_METHOD = input('ガンマ補正方法：(1) LUT、(2) iLUT, (3) fitting', 's');
else
    INPUT_FILE_NAME = [];
    GAMMA_CORRECTION_METHOD = [];
end

PORT_NUM = input('ColorCALのポート番号？[COM3]: ', 's');
if isempty(PORT_NUM)
    PORT_NUM='COM3';
end
...
%     testtexture = Screen( 'MakeTexture', win, testindicies);
    ...
    % infostring = MakeInfoString(infostring,'Cover ColorCAL II, then press space key',handles.infotext);
%     infostring2 = MakeInfoString2(win2, infostring2, 'Cover ColorCAL II, then press any key');
    ...
    ColorCALIIZeroCalibrate(ColorCALIICDCPort); % calibration
