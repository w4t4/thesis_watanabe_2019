% DKL2XYZj (ver1.0)  
% 
% Calculate Judd-modified XYZ values from DKL coodinates.
%
%
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Usage: 
%    lms = DKL2lms(DKL, stdlms, thr, ccmat);
%   
% Input:  
...
% Output:
%   XYZ:    Judd-modified XYZ matrix (the same size as the DKL matrix)
%
% Comments:
...
function XYZ = DKL2XYZj(DKL, stdlms, thr, ccmat)

lms = DKL2lms(DKL, stdlms, thr);
rgb = lms2rgb(lms, ccmat);
XYZ = rgb2XYZj(rgb, ccmat);
