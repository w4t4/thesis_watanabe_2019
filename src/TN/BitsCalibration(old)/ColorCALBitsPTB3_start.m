...
disp('udtmeasBitsPTB3_start.m - last revised 29 June 2007')
disp(' ')
...
%     global udt;
%     if (initUDT < 0)
%         infostring2 = MakeInfoString2(win2, infostring2, 'Error - UDT failed to initialize. You may need to restart matlab.');
%         infostring2 = MakeInfoString2(win2, infostring2, 'Error - UDT failed to initialize. You may need to restart matlab.');
%         infostring2 = MakeInfoString2(win2, infostring2, 'Error - UDT failed to initialize. You may need to restart matlab.');
...
%         closeUDT, return
%     else
%         infostring = MakeInfoString(infostring,'UDT initialization OK!',handles.infotext);
%     end
    ...
    % --- ready? ---
    infostring2 = MakeInfoString2(win2, infostring2, ' ');
    infostring2 = MakeInfoString2(win2, infostring2, 'Set UDT to center spot and press any key to begin measurments...');
    while 1
    ...
    for sample = 1:numSamples
        for n=eval(nrange)
            for gun = eval(grange)
                ...
                ReadCC = colorCALread;
                result(gun, sample, n+1, :) = [x ReadCC(3)];
