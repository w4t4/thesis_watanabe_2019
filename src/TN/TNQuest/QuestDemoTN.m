...
Actualbeta=33;
%beta=1;
delta=0.01;gamma=0.5;
range=tActual.*10;
grain=range/1000;

actualq=QuestCreate_TN(tGuess,tGuessSd,pThreshold,Actualbeta,delta,gamma,grain,range);

trialnum=1000;      % simulaion num
trialsDesired=20;   % trial num for each session
thresholds_TN=zeros(trialnum,1);
thresholds=zeros(trialnum,1);
for i=1:trialnum
    if mod(i, round(trialnum./10))==0
        fprintf('%d:\n',i);
    end
    if i>1
        q=QuestCreate_TN(tGuess,tGuessSd,pThreshold,beta,delta,gamma,grain,range,q);    % for histrical record
    else
        q=QuestCreate_TN(tGuess,tGuessSd,pThreshold,beta,delta,gamma,grain,range);
    end
    ...
    % On each trial we ask Quest to recommend an intensity and we call
    % QuestUpdate to save the result in q
    ...
    for k=1:trialsDesired
        ...
        response=QuestSimulate(actualq,tTest,tActual);
        % fprintf('Trial %3d at %5.2f is %s\n',k,tTest,char(wrongRight(response+1)));
        timeZero=timeZero+eval(getSecsFunction)-timeSplit;

        % Update the pdf
        q=QuestUpdate_TN(q,tTest,response); % Add the new datum (actual test intensity and observer response) to the database.
    end
    ...
    t=QuestMean_TN(q);		% Recommended by Pelli (1989) and King-Smith et al. (1994). Still our favorite.
    sd=QuestSd(q);
    thresholds_TN(i)=t;
    thresholds(i)=QuestMean(q);
end

fprintf('(original)Estimated threshold=%f, SD=%f\n',mean(thresholds),std(thresholds));
fprintf('(TN)Estimated threshold=%f, SD=%f\n',mean(thresholds_TN),std(thresholds_TN));
fprintf('(TN2)Estimated threshold=%f, SD=%f\n',QuestMean_TN(q),QuestSd_TN(q));
